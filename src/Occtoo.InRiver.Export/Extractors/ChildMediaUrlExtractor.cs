...
{
    public class ChildMediaUrlExtractor : IExceptionFieldExtractor
    {
        private readonly Settings _settings;
        private readonly inRiverContext _context;
        private readonly OnboardingServiceClient _serviceClient;

        public ChildMediaUrlExtractor(Settings settings,
        ...
        public void Extract(DynamicEntity dynamicEntity, Entity inRiverEntity, ExceptionFieldSettings settings)
        {
            ...
            if (!ValueHelpers.GetResourceFilename(child, _settings.DocumentIdForbiddenChars, out var filename)) return;

            var occtooMedia = UploadOrGetFileInMediaService(child.MainPictureUrl, id, filename);

            if (occtooMedia == null) return;

            dynamicEntity.Properties.Add(new DynamicProperty
            {
                Id = settings.Alias,
                Language = string.Empty,
                Value = occtooMedia.Url
            });
        }
